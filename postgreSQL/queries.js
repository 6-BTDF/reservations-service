...
const listingsQuery = 'herkbath.listings(dailyPrice,cleaningFee,serviceFee,taxes,holidayPremium,weekendPremium,weeklyDiscount,monthlyDiscount,name,owner,max_guests,min_stay,max_stay)';

const users = path.join(__dirname, 'postgresUsers.csv');
const usersQuery = 'herkbath.users(id,username)';
...
const importFile = (queryDetails, file, bool = false) => {
  client.query(`COPY ${queryDetails} FROM '${file}' DELIMITER ',' CSV HEADER`, (error, results) => {
    if (error) {
      ...
      client.query('ALTER TABLE herkbath.bookings ADD CONSTRAINT fk_listings FOREIGN KEY (id_listings) references herkbath.listings(id) ON DELETE CASCADE', (err, result) => {
        if (error) {
          throw error;
        } else {
          client.query('ALTER TABLE herkbath.bookings ADD CONSTRAINT fk_users FOREIGN KEY (id_users) references herkbath.users(id) ON DELETE CASCADE', (err, result) => {
            if (err) {
              throw err;
            } else {
...
//ALTER TABLE herkbath.listings ADD CONSTRAINT fk_users FOREIGN KEY (owner) references herkbath.users(id) ON DELETE CASCADE;