...
CREATE TABLE herkbath.listings (
  ...
  cleaningFee NUMERIC(3,2) DEFAULT 0 NOT NULL CONSTRAINT positive_cleaningFee CHECK (cleaningFee > 0),
  serviceFee NUMERIC(3,2) DEFAULT 10 NOT NULL CONSTRAINT positive_serviceFee CHECK (serviceFee > 0),
  taxes NUMERIC(2,2) DEFAULT 8 NOT NULL CONSTRAINT positive_taxes CHECK (taxes > 0),
  holidayPremium SMALLINT DEFAULT NULL CONSTRAINT positive_holidayPremium CHECK (holidayPremium >= 0),
  weekendPremium SMALLINT DEFAULT NULL CONSTRAINT positive_weekendPremium CHECK (weekendPremium >= 0),
  weeklyDiscount SMALLINT DEFAULT NULL CONSTRAINT positive_weeklyDiscount CHECK (weeklyDiscount >= 0),
  monthlyDiscount SMALLINT DEFAULT NULL CONSTRAINT positive_monthlyDiscount CHECK (monthlyDiscount >= 0),
...
CREATE TABLE herkbath.bookings (
  ...
  total_price SMALLINT NOT NULL CONSTRAINT positive_price CHECK (total_price > 0),
  ...
  id_listings INT references herkbath.listings(id),
  id_users INT references herkbath.users(id),
  PRIMARY KEY (id)
)
